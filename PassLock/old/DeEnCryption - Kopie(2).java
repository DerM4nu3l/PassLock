...
import dataManagement.PasswordCrypt;
...
public class DeEnCryption {
	 ...
	 *             Cipher passen.
	 */
	public static String checkDecrypt(PasswordCrypt data, String masterPassword, String name) throws Exception {
		byte[] key = generateKey(masterPassword, name);
		byte[] hash = data.getHash();
		if (hash(key).equals(hash)) {
			return decrypt(data.getPassword(), masterPassword, name);
		}
		throw new Exception();
	}
	...
	/**
	 * Liefert den Hash nach dem OpenBSD-Bcrypt-Schema
	 * 
	 * @param password
	 *            Das Passwort, das gehasht wird.
	 * @return Das gehashte Passwort.
	 */
	/*
	 * public static String hashPassword(String password) { return
	 * BCrypt.hashpw(password, BCrypt.gensalt()); }
	 * 
	 * /** Überprüft, ob ein Klartext einem zuvor gehashten entspricht.
	 * 
	 * @param plaintext Der Klartext, der überprüft wird.
	 * 
	 * @param hashed Der Hash mit dem verglichen wird.
	 * 
	 * @return true genau dann, wenn der Hash aus dem Klartext entstanden ist.
	 */
	/*
